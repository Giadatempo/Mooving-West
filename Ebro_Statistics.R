...
setwd("D:/TESI_GIADA/Giada/RIPROIETTATO/EBRO")

#Statistical correlation between Kriging and cost map
##importing raster grids
Ccost_obs <- raster("Cost_map/cost_surface_river_obstacle/cost_cumu.tif")
Ccost_aid <- raster("Cost_map/river/river_cumulative.tif")
CCost_aid_ani <- raster("Cost_map/river_anisotropic/Ebro_basin_cumulative.tif")
no_riv <- raster("Cost_map/no_river/no_riv_cumulative_cost_map.tif")
Krig <- raster("Kriging/Ebro_UniKriging.tif")
...
Stream_dist4 <- raster("distance_matrix_Ebro_Strahler_4_100x100.tif", proj4string = crs.reg)
Stream_dist6 <- raster("distance_matrix_Ebro_Strahler_6_100x100.tif", proj4string = crs.reg)
Coast_dist <- raster("distance_matrix_coast_line.tif", proj4string = crs.reg)
Flint_dist <- raster("distance_matrix_silex.tif", proj4string = crs.reg)
Inland_dist <- raster("distance_matrix_Inlandwater.tif", proj4string = crs.reg)
...
lin_reg5 <- lm(x ~ y + y1 + y2 + y3 + y4 + y5 + y6 + y7, data.r5)
